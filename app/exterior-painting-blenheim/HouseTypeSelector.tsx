...
export default function HouseTypeSelector({ houseTypes }: Props) {
  ...
  const selectedOption = houseTypes.find(o => o.label === selectedHouseType);
    ...
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="hidden lg:block">
        ...
        <div className="relative mb-12 text-center">
          <p className="text-sm text-gray-500 mb-4">Select your house type to see details</p>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              <div className="relative aspect-[4/3] h-[300px] w-full">
                {selectedOption && (
                  <>
                    <Image
                      ...
                      fill
                      priority={houseTypes.indexOf(selectedOption) === 0}
      ...
      <div className="lg:hidden">
        <p className="text-sm text-center text-gray-500 mb-3">Tap a house type to see details</p>
        
        {/* Updated mobile tab container */}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {houseTypes.map((type) => (
              <button
                key={type.label}
                onClick={() => setSelectedHouseType(type.label)}
                className={`relative py-2.5 px-3 rounded-md text-xs font-medium transition-all duration-300 focus:outline-none ${
                  selectedHouseType === type.label
                    ? 'bg-white text-navy shadow-md translate-y-[-1px]'
                    : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
                <span className="uppercase tracking-wide text-[11px] sm:text-xs">{type.label}</span>
                {selectedHouseType === type.label && isClient && (
                  <motion.div
                    className="absolute bottom-0 left-1 right-1 h-0.5 bg-gold"
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="overflow-hidden rounded-lg shadow-md bg-gray-50 relative">
              <div className="relative aspect-[4/3] h-[250px] w-full">
                {selectedOption && (
                  <>
                    <Image
                      src={selectedOption.image}
